...
import builderb0y.bigtech.beams.impl.IgniterBeam;
...
public class IgniterBeamBlock extends BeamBlock implements BlockEntityProvider, LegacyOnStateReplaced {

	public static final MapCodec<IgniterBeamBlock> CODEC = BigTechAutoCodec.callerMapCodec();
	...
	public IgniterBeamBlock(Settings settings) {
		super(settings);
	...
	public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
		return world.isClient ? null : IgniterBlockEntity.SERVER_TICKER.as();
	}
	...
	public @Nullable NamedScreenHandlerFactory createScreenHandlerFactory(BlockState state, World world, BlockPos pos) {
		return world.getBlockEntity(pos) instanceof IgniterBlockEntity blockEntity ? blockEntity : null;
	}
	...
	public boolean onSyncedBlockEvent(BlockState state, World world, BlockPos pos, int type, int data) {
		if (world instanceof ServerWorld serverWorld) {
			...
			if (state.get(Properties.POWERED) && state.get(Properties.LIT)) {
				PersistentBeam newBeam = new IgniterBeam(serverWorld, UUID.randomUUID());
	...
	public void onBlockAdded(BlockState state, World world, BlockPos pos, BlockState oldState, boolean moved) {
		...
		if (world instanceof ServerWorld serverWorld) {
			...
			if (direction != null && direction != this.getFiringDirection(oldState)) {
				PersistentBeam beam = new IgniterBeam(serverWorld, UUID.randomUUID());
	...
	public @Nullable Direction getFiringDirection(BlockState state) {
		return state.isOf(this) && state.get(Properties.POWERED) && state.get(Properties.LIT) ? state.get(Properties.HORIZONTAL_FACING) : null;
	}
