...
public class ComparatorCircuitComponent extends ByteBackedCircuitComponent implements RotatableCircuitComponent {

	public static final BitField
		LEVEL    = BitField.create(4, 0),
		ROTATION = BitField.create(2, LEVEL.nextOffset());
	public static final Canonicalizer<ComparatorCircuitComponent>
		CANONICALIZER = new Canonicalizer<>(1 << ROTATION.nextOffset(), ComparatorCircuitComponent::new);

	public ComparatorCircuitComponent(byte data) {
		super(data);
	}

	public static ComparatorCircuitComponent getInstance(byte data) {
		return CANONICALIZER.get(data);
	}

	public static ComparatorCircuitComponent getInstance(int level, CircuitRotation rotation) {
	...
	public ComparatorCircuitComponent withLevel(@Range(from = 0L, to = 15L) int level) {
	...
	public ItemStack createItemStack() {
		ItemStack stack = new ItemStack(MaterialItems.COMPARATOR_CIRCUIT);
	...
	public CircuitComponent rawTick(int externalInput) {
		...
		return this.withLevel(sideInput > backInput ? 0 : backInput);
	}
}