...
import builderb0y.bigtech.blocks.PistonInteractor;
import builderb0y.bigtech.blocks.PistonInteractor.PistonHandlerInfo;
...
public abstract class PistonHandler_AddNecessaryMethods {
	...
	private static boolean bigtech_isBlockSticky(BlockState state, BlockPos pos, PistonHandler handler) {
		return state.getBlock() instanceof PistonInteractor interactor ? interactor.isSticky(new PistonHandlerInfo((PistonHandlerAccessor)(handler)), pos, state) : isBlockSticky(state);
	}
	...
	private static boolean bigtech_isAdjacentBlockStuck(BlockState state, BlockState otherState, BlockPos pos, BlockPos otherPos, Direction face, PistonHandler handler) {
		return state.getBlock() instanceof PistonInteractor interactor ? interactor.canStickTo(new PistonHandlerInfo((PistonHandlerAccessor)(handler)), pos, state, otherPos, otherState, face) : isAdjacentBlockStuck(state, otherState);
	}
}